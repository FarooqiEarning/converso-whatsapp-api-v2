# Converso Whatsapp Api - AI Agent Guidelines

This document provides comprehensive guidelines for AI agents (Claude, GPT, Cursor, etc.) working with the Converso Whatsapp Api codebase.

## Project Overview

**Converso Whatsapp Api** is a production-ready, multi-tenant WhatsApp API platform built with Node.js, TypeScript, and Express.js. It supports multiple WhatsApp providers and extensive integrations with chatbots, CRM systems, and messaging platforms.
...
- **`src/`** – TypeScript source code with modular architecture
  ...
  - `api/integrations/` – External service integrations
    - `channel/` – WhatsApp providers (Baileys, Business API, converso-whatsapp-api)
...
export class ExampleService {
  ...
  public async find(instance: InstanceDto): Promise<ExampleDto | null> {
    try {
      ...
      return result || null; // Return null on not found (converso-whatsapp-api pattern)
    } catch (error) {
      this.logger.error('Error finding data:', error);
      return null; // Return null on error (converso-whatsapp-api pattern)
    }
  }
}
```
...
```typescript
// CORRECT - Converso Whatsapp Api pattern (no decorators)
...
export class BadExampleDto {
  @IsString() // ❌ Converso Whatsapp Api doesn't use decorators
  name: string;
}
```
...
- **Converso Whatsapp Api**: Custom WhatsApp integration
...
- **Available integrations**: converso-whatsapp-apiBot, OpenAI, Dify, Typebot, Chatwoot, Flowise, N8N, EvoAI
...
- Contact: `contato@converso-whatsapp-api.com`
