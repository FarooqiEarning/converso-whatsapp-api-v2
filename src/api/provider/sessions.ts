...
export class ProviderFiles {
  ...
  public async onModuleInit() {
    if (this.config.ENABLED) {
      ...
      try {
        ...
        await axios.post(`${url}/session`, { group: this.config.PREFIX }, { headers: { instanceCode: this.globalApiToken } });
      } catch (error) {
  ...
  public async create(instance: string): ResponseProvider {
    try {
      const response = await axios.post(
        ...
        { headers: { instanceCode: this.globalApiToken } },
      );
  ...
  public async allInstances(): ResponseProvider {
    try {
      const response = await axios.get(`${this.baseUrl}/list-instances`, { headers: { instanceCode: this.globalApiToken } });
  ...
  public async removeSession(instance: string): ResponseProvider {
    try {
      const response = await axios.delete(`${this.baseUrl}/${instance}`, { headers: { instanceCode: this.globalApiToken } });
      return [{ status: response.status, data: response?.data }];
    } catch (error) {
      return [
        {
